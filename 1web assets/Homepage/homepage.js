...
function setup() {
  ...
  if (e.key === "Escape") {
    window.close(); // This will close the Electron app
  }
});
...
function initPoseNet() {
  // Request camera permissions even when in background
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(function(stream) {
        // Continue with PoseNet initialization
        videoFeed = createCapture(VIDEO);
        videoFeed.size(640, 480);
        videoFeed.hide();
...
//   window.electronAPI.notify("Awas Postur!", "Kamu terdeteksi membungkuk, segera perbaiki posturmu","");
// }}
    ...
    if (Notification.permission === "granted") {
      ...
      Notification.requestPermission().then(permission => {
        if (permission === "granted") {
          new Notification("Awas Postur!", {
            body: "Kamu terdeteksi membungkuk, segera perbaiki posturmu!",
            icon: "1web assets/Homepage/Assets/notif-icon.jpeg"
          });
          lastNotificationTime = now;
        }
      });
